...
struct Cluster
    ...
    ranks::Int
    layout_cores::Int
    possible_atm_layouts
    ice_layout::Tuple{Int, Int}
...
determine_io(α::Int) = mod(α,4) == 0 ? 4 : mod(α,3) == 0 ? 3 : mod(α,2) == 0 ? 2 : 1
...
function make_cluster(ρ::ResourceTag, κ::String)
    ...
    println(typeof(ranks))
    layout_cores = div(ranks, 6)
    println(typeof(layout_cores))
    possible_atm_layouts = atm_factors(layout_cores)
    ice_layout = ice_factors(ranks)[1]
...
#    matched_atm_layouts = self.match_layouts() 
end
